...
for htmlfile in htmlfiles:
    ...
    for item in items:
        ...
        while True:
            ...
            if next_element == '\n':
                ...
                break
            elif next_element.has_attr("class") and (
                    next_element["class"][0] == "stat-block-title"):
                break
            else:
    ...
    for element in all_elements:
        ...
        for tag in element:
            if crafting_flag:
                ...
                if (tag.has_attr("class") and
                        tag["class"][0] == "stat-block") or (
                            not tag.has_attr("class")):
                    tag.attrs = dict()
                    desc += str(tag)
                elif tag.has_attr("class") and tag["class"][
                        0] == "stat-block-1":
        ...
        extract_info = re.search(
            'Slot\s?(.*?);\s*Aura\s?(.*?);\sCL\s?([0-9]*?)[a-z]*; Weight\s?(.*?)\slb',
            details)
        if extract_info != None:
            item_contents["Slot"] = extract_info.groups()[0].replace(',', '')
            item_contents["Aura"] = extract_info.groups()[1]
            item_contents["CL"] = extract_info.groups()[2]
            item_contents["Weight"] = extract_info.groups()[3]
        else:
            extract_info = re.search('Slot\s?(.*?);\sAura\s?(.*?);', details)
            print(details)
            item_contents["Slot"] = extract_info.groups()[0].replace(',', '')
            item_contents["Aura"] = extract_info.groups()[1]
